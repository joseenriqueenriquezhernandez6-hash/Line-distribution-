...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .member {
    ...
    border:1px solid rgba(255,255,255,0.02); /* borde más sutil */
  ...
  <div class="panel">
    <div class="row" style="align-items:center">
      <div style="min-width:200px">
        <label style="display:block;font-size:13px;margin-bottom:6px">Duración canción (min : seg)</label>
      ...
      <div style="min-width:220px">
        <label style="display:block;font-size:13px;margin-bottom:6px">Duración total de la línea (segundos) — max para barras</label>
    ...
    <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap">
      <button class="btn-neutral" onclick="saveAll()">Guardar distribución</button>
      <button class="btn-neutral" onclick="loadAll()">Cargar última</button>
...
function resetAll(){
  if(!confirm("¿Reiniciar TODO? Se borrará todo y el guardado.")) return;
  clearAllIntervals();
  members = []; localStorage.removeItem(STORAGE_KEY);
  render();
}
function resetDistribution(){
  if(!confirm("¿Borrar integrantes y tiempos (mantener ajustes)?")) return;
  clearAllIntervals();
  members = []; render();
}
  ...
  members.forEach(m => {
    ...
    card.innerHTML = `
      <div class="member-top">
        <div class="left">
          ...
          <div class="info">
            ...
            <div class="meta">${m.time.toFixed(1)}s • ${percent.toFixed(1)}%</div>
          </div>
        </div>
      ...
      <div class="controls">
        ...
        <button class="disableBtn">${m.disabled? '🔒':'➖'}</button>
